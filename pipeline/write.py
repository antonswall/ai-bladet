...
def sonnet_call(prompt: str, system: str = None,
    ...
    """Anropa Claude Sonnet 4.6 via OpenRouter, med sessionslös Claude CLI-fallback."""
    key = _get_openrouter_key()
    if not key:
        print("  ⚠️  OPENROUTER_API_KEY saknas — försöker sessionslös CLI-fallback", file=sys.stderr)
        return subscription_write_fallback(prompt, system, max_tokens=max_tokens)
    ...
    try:
        ...
        print("  ↪ Försöker sessionslös Claude Sonnet-fallback (ingen API-debitering)", file=sys.stderr)
        return subscription_write_fallback(prompt, system, max_tokens=max_tokens)


def subscription_write_fallback(prompt: str, system: str = None,
                                max_tokens: int = MAX_OUTPUT_TOKENS) -> Optional[str]:
    """Skrivstegs-fallback via pipeline/llm.py när OpenRouter/Sonnet är otillgängligt."""
    if llm_call is None:
        print("  ❌ CLI-fallback saknas: kunde inte importera llm_call", file=sys.stderr)
        return None
    strict_system = (system or "") + """

VIKTIGT I DETTA FALL: Du kör via den sessionslösa CLI-fallbacken eftersom OpenRouter inte svarar.
...
"""
    return llm_call(
        ...
        timeout=300,
        attempts=2,
        model="sonnet",  # skrivsteget får Sonnet; Opus är spärrat (se llm.py)
    )
