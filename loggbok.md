...
- **Varför det inte upptäcktes:** `post-to-moltbook.py` betraktade lyckad math-verifiering som leveransbevis och kollade aldrig `is_spam` eller flödesnärvaro → tyst falsk success. `already_published()` matchade dessutom på `verification_status`, ett fält `/search` inte returnerar, så den returnerade alltid `None` och postade om varje körning (3 dubbletter för vecka 35).
- **Andra verifierade fel:** (1) Moltbook var en hård grind före distributionen i `run_weekly.sh` — därför tappade söndagskörningen 2026-08-30 audio, meme, podcast och SeenDB-commit. (2) `write.py` daterade varje söndagskört nummer en vecka för tidigt (`now - (weekday+1)` drar av 7 dagar på söndagar); vecka 25–32 ligger kvar feldaterade. (3) Podcastfeedens enclosure-URL:er pekade på `https://aibladet.se`, som är parkerad hos one.com (A 46.30.211.38, ingen giltig TLS) — alla avsnitt var onedladdningsbara.
- **Latent haveri som avblockeringen avslöjade:** `run_weekly.sh` startade `distribute.py` och `post-to-moltbook.py` med `python3`, inte `python`. `python3` = homebrew 3.14.6 som saknar `requests`/`yaml` och har trasig `pyexpat`; `python` = `pipeline/.venv` 3.11.15 som preflight validerar. `distribute.py` startar sina moduler med `sys.executable`, så hela distributionen hade dött på `ModuleNotFoundError: requests` så fort Moltbook slutade kortsluta körningen. Båda anropen använder nu `python`.
- **Ändringar:** `pipeline/post-to-moltbook.py` (`post_is_visible`/`in_public_feed`/`get_post`, exit 2 vid osynlig post, `already_published` kräver nu verified + inte spam); `pipeline/run_weekly.sh` (Moltbook blockerar inte längre distributionen, men körningen exitar 1 ändå); `pipeline/write.py` (datum = söndagen i utgåvans egen ISO-vecka via `date.fromisocalendar`); `pipeline/distribute_audio.py` + `public/feed/podcast.xml` (canonical host); `content/2026-34.md` (datum 2026-08-16 → 2026-08-23; en okommitterad arbetskopia hade dessutom skrivit över vecka 34 med vecka 35:s innehåll — återställd).
- **Tester:** `py_compile` ×3, `bash -n`, 31/31 unittester gröna (14 nya: datumhärledning, Moltbook-synlighet, runner-kontrollflöde, tolkval, podcast-host), `node build.js` (11 utgåvor), `git diff --check`.
- **Livebevis efter push:** se commit nedan + `/tmp/ai-bladet-v35-fix-resultat.md`.
- **Kvar till lutra/Anton:** (a) Moltbook-spamflaggan går inte att häva via API:t (inget `/spam`- eller appeal-endpoint, inget skäl exponeras) — kräver kontakt med Moltbook eller manuell moderering; jag postade medvetet INTE en fjärde post eftersom 3/3 redan flaggats och en blind omposting bara förstärker signalen. (b) Vecka 25–32 är fortfarande feldaterade i `content/`; backfill medvetet ej gjord (ändrar 8 pubDate i RSS). (c) `templates/issue.js` och `build.js` sätter canonical/og till `https://aibladet.se` — bör pekas om till canonical host.
